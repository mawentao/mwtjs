...
{
	var container=mwt.create_div('mwt-notify-container');
	if(msg) {
		var divid=mwt.gen_domid('mwt-notify-div-');
		if (!cls) cls='';
        var icon = '';
		switch(cls) {
...
// 销毁全部消息
mwt.notify_clearall=function(){
	jQuery('#mwt-notify-container').html('');
};
...
mwt.show_progress = function(progress)
{
    var divid = MWT.create_div('mwt-progress-div');
    var pid = 'loading-progress-span';
    if (!document.getElementById(pid)) {
        var code = "<span id='loading-progress-span' "+
            "style='top:0;left:0px;width:0%;height:2px;position:absolute;z-index:999;border-radius:2px;background-color:"+MWT.progress_color+";'></span>";
	...
	setTimeout(function(){
		MWT.show_progress(progress);
	}, 500);
};
// 隐藏进度条
mwt.hide_progress = function()
{
    var data = {width:"100%"};
