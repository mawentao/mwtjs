...
MWT.DatepickerField=function(opt)
{
    this.listeners={};
    ...
    if (opt)
	{
        ...
        if(isset(opt.value)) {
            var tm = time();
            if (is_numeric(opt.value)) {
                tm += 86400 * opt.value;
            } else {
                tm = strtotime(opt.value);
            }
            var dt = date("Y/m/d",tm);
            this.value=new Date(dt).format(this.format);
        }
	}
    // create
    this.create=function(){
        ...
        var code="<input type='text' class='mwt-field mwt-datepicker'";
