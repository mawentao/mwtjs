...
MWT.CheckboxField=function(opt)
{
    ...
    this.create=function(){
        ...
        for (var i=0;i<options.length;++i) {
            var item=options[i];
            var cb='<label class="'+this.cls+'">'+
                '<input type="checkbox" name="'+rdoname+'" value="'+item.value+'"><span style="float:right;padding-left:1px;">'+item.text+'</span></label>';
            rdoes.push(cb);
        }
        var code = rdoes.join("&nbsp;&nbsp;");
        ...
        jQuery('[name='+rdoname+']').change(function(){
            thiso.value = mwt.get_checkbox_values(rdoname);
            thiso.fire("change");
        });
        // set default
        this.reset();
    ...
    this.setValue=function(v){
        this.value=v;
        mwt.set_checkbox_values(rdoname,v);
    };
